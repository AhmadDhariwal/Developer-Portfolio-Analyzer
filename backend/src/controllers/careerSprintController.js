...
const createCareerSprint = async (req, res) => {
  try {
    const { title, weeklyGoal, tasks } = req.body || {};
    const sprint = await createSprint({ userId: req.user._id, title, weeklyGoal, tasks });
...
const getCareerSprintHistory = async (req, res) => {
  try {
    ...
    const history = await CareerSprint.find({ userId: req.user._id }).sort({ weekStartDate: -1 }).limit(limit).lean();
    res.json({ history });
  } catch (error) {
