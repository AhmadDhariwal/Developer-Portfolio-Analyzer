...
export class SaDevelopersComponent implements OnInit {
  ...
  load(page = 1): void {
    ...
    this.sa.getDevelopers(params).subscribe({
      next: (res) => { this.developers = res.developers || []; this.total = res.total || 0; this.totalPages = res.totalPages || 1; this.loading = false; },
