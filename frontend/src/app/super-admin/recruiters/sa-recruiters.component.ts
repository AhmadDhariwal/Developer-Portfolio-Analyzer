...
export class SaRecruitersComponent implements OnInit {
  ...
  load(page = 1): void {
    ...
    this.sa.getRecruiters(params).subscribe({
      next: (res) => { this.recruiters = res.recruiters || []; this.total = res.total || 0; this.totalPages = res.totalPages || 1; this.loading = false; },
